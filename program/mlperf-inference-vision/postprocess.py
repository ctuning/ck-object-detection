...
def ck_postprocess(i):
  ...
  ck.out('mAP=%.3f%% (from the postprocessing script)' % save_dict['mAP_pc'])
  ...
  for scenario in [ 'SingleStream', 'MultiStream', 'Server', 'Offline' ]:
    ...
    scenario = save_dict['output'].get(scenario_key, None)
    if scenario:
        ck.out('mAP=%.3f%% (from the output for %s)' %
              (scenario.get('mAP', 0.0) * 100.0, scenario_key))
